...
// ---------------------------------------------------------------------------
// The user's home directory ($HOME). Empty if unset; callers that need a
// concrete path fall back to a tilde-less absolute only when HOME is absent.
...
// Expand a config-file db path: a leading "~" becomes the user's home, and
// $VAR / ${VAR} references are read from the environment (notably $HOME), so
// a portable "$HOME/.config/hax/memory.dl" line resolves on any host. Returns
// an owned allocation (caller keeps it for the process lifetime, like the
// raw-config value it replaces) or null on OOM.
...
fn config_db_path() ?[]const u8 {
    ...
    while (true) {
        ...
        // Expand "~" / "$HOME" (etc.) so a portable config line works anywhere.
...
// $XDG_CONFIG_HOME default: $HOME/.config (the XDG spec). Home-derived so the
// binary is portable across hosts, not baked to a build-time user.
...
const usage =
    ...
    \\
    \\db: $FX_AGENT_MEMORY_DB | $JING_MEMORY_DB | config file ($FX_AGENT_MEMORY_CONFIG | $XDG_CONFIG_HOME/hax/fx-agent-memory | ~/.config/hax/fx-agent-memory) | $HOME/.jing/memory.dl
    \\
;
